...
def donor_medical_card(request, donor_id):
    ...
    total_donations = 16
    # Get the latest directions (for the "Выдано направление" links)
    latest_directions = DirectionBase.objects.filter(donor=donor).order_by('-created_at')[:3]

    # Get active directions for the directions list
    active_directions = DirectionBase.objects.filter(
        donor=donor,
        is_completed=False,
        expiry_date__gte=timezone.now().date()
    ).order_by('expiry_date')[:2]  # Limit to 2 for the design
    ...
    context = {
        ...
        'latest_directions': latest_directions,
...
def donation_view(request, donation_id):
    ...
    context = {
        ...
        'available_dates': available_dates,
        'read_only': True,
        'is_edit_allowed': True  # You can control this based on permissions
    }
...
def donor_card_create(request):
    ...
    # GET request - show the form
    donors = Donor.objects.all().order_by('last_name', 'first_name')
    return render(request, 'donor_card_form.html', {'donors': donors})
...
def donor_card_edit(request, card_id):
    ...
    if request.method == 'POST':
        try:
            ...
            donor_card.has_contraindications = request.POST.get('has_contraindications')
